...
screening to minimise the probability of contamination. This graph shows that this condition is met provided the probability of use is greater after contact is made ($\rho<\omega$). This is entirely predictable but suggests that we now need to think about the factors that impact adherence before and after and how they might be modelled.

# Hand hygiene with chance of adherenece increasing with threat

Here we consider adherence increasing with threat and so define a efficacy of intervention and a percevied threat. Using the tray boosts the threat perception and so the probability of uptake of intervention is always greater after touching trays.
```{r increasingthreat}
nsim<- 1000
...
Here we recreate a version of Figure 4 in Walters et al.
```{r hprev}
prevalence <- 0.5
...
ggplot(data=comp)+
     geom_contour_filled(mapping=aes(y=eps, x=tau, z=theta))
ggplot(data=comp)+
